...
//     await page.locator('input[name="username"]').fill('Sahae');
//     // await page.locator('input[name="password"]').fill('Sahas');
...
//     await page.locator('[name="customer.firstName"]').waitFor();                                
//     //  await page.waitForLoadState('networkidle'); -- alternavtive for loading the page interactioN
...
test('register new customer' ,async ({page}) =>{
    ...
    await registerPage.register('sahasee','rt','ttr','twtrsd','etre3','tr53','e55','y535','sahasee_${Date.now()}','sahasee','sahasee');
    await expect(page.locator('h1')).toBeVisible();
    await expect(page.locator('h1')).toContainText('Welcome');
});
